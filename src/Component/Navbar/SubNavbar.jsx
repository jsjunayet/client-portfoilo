...
const SubNavbar = () => {
    ...
    useEffect(() => {
        ...
        const observerOptions = {
            root: null,
            rootMargin: "0px",
            threshold: 0, // 30% দৃশ্যমান হলে সক্রিয় সেকশন
        };
    ...
    // useEffect(() => {
    //     const handleScroll = () => {
    //         if (!isDynamicPath) return; 
    //         const currentScrollPos = window.scrollY;
    //         const isScrollingDown = currentScrollPos > prevScrollPos;

    //         const threshold = 78; // স্ক্রল সীমা

    //         if (isScrollingDown && currentScrollPos > threshold) {
    //             controls.start({ opacity: 0, y: -50 }); // হাইড করার এনিমেশন
    //         } else {
    //             controls.start({ opacity: 1, y: 0 }); // দেখানোর এনিমেশন
    //         }
    ...
    //     return () => {
    //         window.removeEventListener("scroll", handleScroll); // ক্লিনআপ
    //     };
                    ...
                    {navLinks.map((link) => (
                        ...
                        >
                            <p
                                ...
                                className={`text-white px-1 py-2 rounded-md cursor-pointer ${
                                    activeSection === link.id ? "border-b-2 border-blue-200" : ""
                                }`}
                            >
                                {link.title}
                            </p>
